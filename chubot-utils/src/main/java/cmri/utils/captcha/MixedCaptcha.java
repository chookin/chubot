 ...
 */
public class MixedCaptcha extends CaptchaGenerator{
    /**
     * 数字和大写字母的生成字典
     */
    ...
    public MixedCaptcha(int codeNumber, int width, int height, int fontSize) {
    ...
    void drawGraphic(BufferedImage image){
        ...
        for(int i=0;i<codeNumber;i++) {
            ...
            // 设置字体旋转角度
            int degree = random.nextInt() % 45 * (random.nextInt(2)*2 - 1);
            // 正向角度
     ...
     */
    String generateChineseWordByGBK(Random rand){
        while (true) {
    ...
    String generateChineseByDict(Random rand){
