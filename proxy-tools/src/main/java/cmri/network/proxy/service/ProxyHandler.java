...
public class ProxyHandler extends BaseOper {
    ...
    private static Set<Proxy> getDBProxies() {
        ProxyDAO dao = ProxyDAO.getInstance();
        try {
            QueryBuilder queryBuilder = new QueryBuilder()
                    .put("properties.highAnonymity").is(true)
                    ;
            List<Proxy> proxies = ProxyDAO.getInstance().find(queryBuilder.get());
            return new HashSet<>(proxies);
        } finally {
            dao.close();
        }
    }
    ...
    /**
     * @return true if success to access network by this proxy. On return, the new proxy is in active.
     */
    public static boolean validateProxy(Proxy proxy, String url) {
        if (checkNetwork(proxy, url)) {
            ...
            proxy.set("validateTime", new Date());
            return true;
        } else {
